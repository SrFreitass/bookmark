...
class AuthController {
    constructor(private readonly app: typeof App) {
        ...
        this.app.post("/api/v1/auth/signin", async (context) => {
            try {
        ...
        this.app.post("/api/v1/auth/refreshtoken", async (context) => {
            try {
                const usecase = new RefreshTokenUseCase(new UserRepositoryImpl(db, users), context.jwt)
